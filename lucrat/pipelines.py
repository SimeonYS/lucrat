...
class DatabasePipeline:
    # Database setup
    conn = sqlite3.connect('lucrat.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute(""" CREATE TABLE IF NOT EXISTS articles 
        (title text, date text, author text, category text, tags text, link text, content text) """)
    ...
    def process_item(self, item, spider):
        ...
        # Insert values
        self.c.execute("INSERT INTO articles (title, date, author, category, tags, link, content)"
                       " VALUES (?,?,?,?,?,?,?)", (item.get('title')[0], item.get('date')[0], item.get('author')[0],
                                                   item.get('category')[0], item.get('tags')[0],
                                                   item.get('link')[0], item.get('content')[0]))
